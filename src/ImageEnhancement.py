...
def enhanceImage(iris_normalized, block_size=16, region_size=32):
    """
    This function implements the Image Enhancement strategy from Section 3.2.3 of Ma et al. (2003).
    Normalized images often suffer from low contrast and non-uniform illumination. This script fixes this in 3 steps:
    1. It calculates the mean intensity of every 16x16 block to get a coarse illumination map.
    2. It expands this coarse map to the original image size using bicubic interpolation and subtracts it from the normalized image.
    3. It applies OpenCV's local histogram equalization to every 32x32 region of the compensated image to obtain a well-distributed texture.

    Parameters:
    - iris_normalized (numpy.ndarray): The 2D grayscale unwrapped iris image (e.g., 64x512).
    - block_size (int): Size of the blocks used for coarse background estimation (default 16 as per Ma et al.).
    - region_size (int): Size of the local regions used for histogram equalization (default 32 as per Ma et al.).

    Returns:
    - tuple: (rough_illumination, illumination, adjusted, enhanced). The final processed image is the 4th element ('enhanced').
    """
    #Get dimensions
